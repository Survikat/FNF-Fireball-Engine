...
final class GameManager {
    ...
    public static function playMusic(manager:SongManager, title:String, ?instOnly:Bool = false, ?dualVocals:Bool = true):Map<FunkinTracks, FlxSound> {
        ...
        instTrack.load(Resources.getSound('songs/$title/Inst'));
        tracks.set(Instrumental, instTrack);

        var songMeta:SongMetaData = Json.parse(Resources.getContent('songs/$title/$title.json'));
        ...
        if (!instOnly) {
            if (dualVocals) {
                vocalTrackPlayer.load(Resources.getSound('songs/$title/Vocals-Player'));
                tracks.set(Player, vocalTrackPlayer);

                vocalTrackOpponent.load(Resources.getSound('songs/$title/Vocals-Opponent'));
                tracks.set(Opponent, vocalTrackOpponent);

                manager.addTrack(vocalTrackPlayer);
                manager.addTrack(vocalTrackOpponent);
            } else {
                vocalTrackPlayer.load(Resources.getSound('songs/$title/Vocals'));
